<?php
	...
	*/
	...
	use \Datos\Repositorio\Habilitacion\CotitularRepo;
	...
	class Tramites{
		...
		*/
		public static function eliminarAdjunto($nroTramite, $ruta){
			$tramites=AdjuntoTramiteRepo::eliminarAdjunto($nroTramite, $ruta);
		...
		*/
		public static function ValidarAgenciaySubagencia($nroTramite){
			$categoria=CategoriaRepo::ValidarAgenciaySubagencia($nroTramite);
			return $categoria;
		}
		...
		*/
		...
		/***************************************/
		public function estadosPosibles($estadoInicial, $usuarioLogueado, $tipoTramite){
			$estadosPosibles = EstadoTramiteRepo::estadosPosibles($estadoInicial, $usuarioLogueado, $tipoTramite);
		...
		**/
		...
		/**
		 * Devuelve una lista de los Tipo Relacion
		 */
		public function listaTipoRelacion(){
			$lista = SuiteCRMRepo::TiposRelacion();
			return $lista;
		}
		 ...
		 * Devuelve una lista de los Tipo Relacion
		 */
		public function listaTipoVinculo(){
			$lista = SuiteCRMRepo::TiposVinculo();
			return $lista;
		}
		...
		*/
		public function datosPermisoSuitecrm($nroPermiso, $tipoTramite){
			...
			//\Log::info("datosPermisoSuitecrm", array($datos));
...
/**************************************Métodos específicos para carga de trámites***************************************/
		...
		**/
		public function modificarEstadoTramite($idTramite, $idEstadoI, $idEstadoF, $observaciones, &$datos_mail, $esNuevo){
			...
			// si es estadof == 1 y tipo de tramite 2 controlar agencia y subagencia para cambio de categoria
			if($idEstadoF == 1 && !$esNuevo){// falta tipo de tramite
				// obtener tipo de tramite
				$tramite = TramiteRepo::buscarPorId($idTramite);				
				 \Log::info("MM - modificarEstadoTramite - tramite:", array($tramite->tipoTramite['id_tipo_tramite']));
						if($tramite->tipoTramite['id_tipo_tramite']==2){
							$cargaAgeySubage = CategoriaRepo::ValidarAgenciaySubagencia($idTramite);
							\Log::info("MM - modificarEstadoTramite - cargaAgeySubage:", array($cargaAgeySubage));
							if(!$cargaAgeySubage){
								$mensaje = "Debe completar en Modificar Trámite la Agencia/Subagencia antes de seguir .";
								return $mensaje;
							}
						}
			}
		...
		// MMery
		// actualizaCorreo
		public function actualizaCorreo($nuevo_mail, $nro_permiso, $usuario){
		
		
		$valido = TramiteRepo::actualizaCorreo($nuevo_mail, $nro_permiso, $usuario);
		...
		**/
		public function modificarEstadoTramiteAR(
			$idTramite,
			$idEstadoI,
			$idEstadoF,
			$datoExtra=NULL ,
			...
			\Log::info("MM - modificarEstadoTramiteAR - idTramite", array($idTramite));
			\Log::info("MM - modificarEstadoTramiteAR - idEstadoI", array($idEstadoI));
			\Log::info("MM - modificarEstadoTramiteAR - idEstadoF", array($idEstadoF));
			\Log::info("MM - modificarEstadoTramiteAR - datoExtra", array($datoExtra));
			\Log::info("MM - modificarEstadoTramiteAR - resolucion", array($resolucion));
			\Log::info("MM - modificarEstadoTramiteAR - expediente", array($expediente));
			\Log::info("MM - modificarEstadoTramiteAR - datos_mail", array($datos_mail));
			\Log::info("MM - modificarEstadoTramiteAR - tipoTramite", array($tipoTramite));
			\Log::info("MM - modificarEstadoTramiteAR - subagente", array($subagente));
			\Log::info("MM - modificarEstadoTramiteAR - esNuevo", array($esNuevo));
			...
			switch ($tipoTramite) {
				...
				case '2':
					...
					$todoCompleto = CategoriaRepo::CompletoTodosLosCampos($idTramite);
						...
						// \Log::info("MM - modificarEstadoTramiteAR - todo):", array($todo));
						// \Log::info("MM - modificarEstadoTramiteAR - idEstadoF):", array($idEstadoF));
						// if($idEstadoF==1){
							// $cargaAgeySubage = CategoriaRepo::ValidarAgenciaySubagencia($idTramite);
							// \Log::info("MM - modificarEstadoTramiteAR - cargaAgeySubage):", array($cargaAgeySubage));
							// if($todo && $cargaAgeySubage){
								// $todoCompleto = true;
							// }else{
								// $todoCompleto = false;
							// }
				...
				case '4':
					...
					break;
				case '11': //baja cotitular
					$todoCompleto = true;
					break;
				case '12': //fallecimiento
					$todoCompleto = true;
					break;
				default:
		...
		public function cargarDomicilio($datos, &$datos_mail) {
			...
			\log::info('MM - cargarDomicilio');
		 	$tramite = null;
		 	if(\Session::has('tramiteNuevoPermiso')){
				// \log::info('MM - cargarDomicilio - entra a if tramiteNuevoPermiso');
		 		$tramiteNuevoPermiso=\Session::get('tramiteNuevoPermiso');
				// \log::info('MM - cargarDomicilio - $tramiteNuevoPermiso.nro_tramite: ', array($tramiteNuevoPermiso['nro_tramite']));
		 		$tramite = TramiteRepo::buscarPorId($tramiteNuevoPermiso['nro_tramite']);
				// \log::info('MM - cargarDomicilio - $tramiteNuevoPermiso.nro_tramite: ', array($tramite));
				...
				// \log::info('MM - cargarDomicilio - entra a else');
				$tramite=self::armaTramite($datos);
				$tramite = TramiteRepo::crear($tramite);
		 	}

			// \log::info('MM - tramite - cargarDomicilio - tramite: ',array($tramite));
			$domicilio=self::armaDomicilio($tramite,$datos);
			// \log::info('MM - cargarDomicilio - domicilio:',array($domicilio));
			...
			if (\File::isDirectory($directorioAdjuntos)){//si existe el directorio
				...
				if (is_dir($directorioAdjuntos)){
					...
					foreach ($archivos as $archivo)
					{
						...
						// por imagen revisar el prefijo - enviar a funcion($archivo->getFilename())  retorna el id
						// \log::info('MM - cargarDomicilio - nombre archivo:',array($archivo->getFilename()));
						\log::info('MM - cargarDomicilio - nombre archivo substr:',array(substr($archivo->getFilename(), 0, 3)));
						
						
						$tiposDocumentos = $this->servicioTipoDocumento->buscarTodos(); 
						\log::info('MM - cargarDomicilio - nombre archivo substr:',array($tiposDocumentos));
						// $idPrefijo=TipoDocumentoRepo::buscarPorPrefijo(substr($archivo->getFilename(), 0, 3)); // buscarPorPrefijoLike
						$iddelprefijo3 = $this->servicioTipoDocumento->buscarPorPrefijo(substr($archivo->getFilename(), 0, 3)); 				 
						$idPrefijo = $iddelprefijo3[0]->idTipoDoc;
						\Log::info("MM - cargarDomicilio - iddelprefijo3:", array($iddelprefijo3[0]->idTipoDoc)); 
						
						$adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta, $idPrefijo);
						// $adjuntos[] = self::armaAdjuntoConTipoDoc($tramite->nroTramite,$tramite->nroPermiso, $ruta, $idPrefijo->idTipoDoc);
						// if(is_null($idPrefijo)){
						\log::info('MM - cargarDomicilio - armaAdjunto - regreso');	
						// }else{						
		...
		// \Log::info('MM - cargarPlan parametros cargarPlan:',array($datos));
		// \Log::info('MM - cargarPlan parametros nroTramite:',array($nroTramite));
			...
			if (\File::isDirectory($directorioAdjuntos)){//si existe el directorio
				...
				if (is_dir($directorioAdjuntos)){
					...
					foreach ($archivos as $archivo)
					{
						...
						$iddelprefijo3 = $this->servicioTipoDocumento->buscarPorPrefijo(substr($archivo->getFilename(), 0, 3)); 				 
						$idPrefijo = $iddelprefijo3[0]->idTipoDoc;
						\Log::info("MM - cargarDependencia - iddelprefijo3:", array($iddelprefijo3[0]->idTipoDoc)); 
						
						// $adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta, $idPrefijo);
						$ruta = $directorioAdjuntos.$ds.$archivo->getFilename();
						// $adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta, 0);
						$adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta, $idPrefijo);
					}
		...
		public function cargarCategoria($datos, &$datos_mail) {
			...
			if (\File::isDirectory($directorioAdjuntos)){//si existe el directorio
				...
				foreach ($archivos as $archivo)
				{
					$iddelprefijo3 = $this->servicioTipoDocumento->buscarPorPrefijo(substr($archivo->getFilename(), 0, 3)); 				 
					$idPrefijo = $iddelprefijo3[0]->idTipoDoc;
					\Log::info("MM - cargarCategoria - iddelprefijo3:", array($iddelprefijo3[0]->idTipoDoc)); 
					$ruta = $directorioAdjuntos.$ds.$archivo->getFilename();
					// $adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta, 0);
					$adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta, $idPrefijo);
				}
		...
		**/
		...
		public function cargarTitular($datos, &$datos_mail, $tipoTramite) {
			...
			if($tipoTramite == 8){
				$separador = "IT_";
			}elseif($tipoTramite == 12){
				$separador = "FT_";
			} 

			if(isset($titular)){

				\Log::info('AE - servicio->cargarTitular - titular: ', array($titular));
				...
				if (\File::isDirectory($directorioAdjuntos)){//si existe el directorio
					...
					foreach ($archivos as $archivo)
					{
						$iddelprefijo3 = $this->servicioTipoDocumento->buscarPorPrefijo(substr($archivo->getFilename(), 0, 3)); 				 
						$idPrefijo = $iddelprefijo3[0]->idTipoDoc;
						\Log::info("MM - cargarTitular - iddelprefijo3:", array($iddelprefijo3[0]->idTipoDoc)); 
						$ruta = $directorioAdjuntos.$ds.$archivo->getFilename();
						// $adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta , 0);
						$adjuntos[] = self::armaAdjunto($tramite->nroTramite,$tramite->nroPermiso, $ruta , $idPrefijo);
					}
...
/**********************************************Para consulta****************************************************************/
		...
		/*	JG-18/11/2020				*/
		/********************************/
		public function listarTramitesPendientes($nroPermiso){
			$eval = TramiteRepo::listadoTramitesPendientes($nroPermiso);
			return $eval;
		}
/************************************FIN Consultar Tramites Pendientes************************************/
	    ...
	    public function armarFiltroTramite($filtros){
			...
			$filtro->localidad = $filtros['localidad'];
	        return $filtro;
	    }
...
/***************************** Armar entidades *************************************/
		...
		*/
		private function armaTramite($datos){
			...
			\Log::info("MM - tramite-tipoTramite: ",array($tramite->tipoTramite)); 
			\Log::info("MM - tramite-estadoTramite: ",array($tramite->estadoTramite)); 
			...
			}elseif(\Session::has('tramiteNuevoPermiso')){
				\Log::info("MM - servicio incorporarMaquina - entra al elseif es tram nuevo"); 
		
				$agenteBolsa=self::obtenerAgenteBolsa();
				\Log::info("MM - servicio incorporarMaquina - entra al elseif es tram nuevo - agenteBolsa",array($agenteBolsa)); 
				$redAgenteBolsa=self::obtenerRedAgenteBolsa();
				\Log::info("MM - servicio incorporarMaquina - entra al elseif es tram nuevo - redAgenteBolsa",array($redAgenteBolsa)); 
				$domicilioAgenteBolsa=self::obtenerDireccionAgenteBolsa();
				\Log::info("MM - servicio incorporarMaquina - entra al elseif es tram nuevo - domicilioAgenteBolsa",array($domicilioAgenteBolsa)); 
				$tramiteNuevoPermiso=\Session::get('tramiteNuevoPermiso');
				\Log::info("MM - servicio incorporarMaquina - entra al elseif es tram nuevo - tramiteNuevoPermiso",array($tramiteNuevoPermiso)); 
...
\Log::info("MM - entra al elseif es tram nuevo - fin - tramite",array($tramite)); 
\Log::info("MM - entra al elseif es tram nuevo - fin"); 
			}else{//registra el cas para un agente/subagente en particular
				$usuarioTramite=\Session::get('usuarioTramite');
\Log::info("MM tramite usuarioTramite",array($usuarioTramite));
				$tramite->usuario = $usr['id'];
				...
				$tramite->titular = $usuarioTramite['titular'];
			...
			if($datos['id_tipo_tramite'] == 6 || $datos['id_tipo_tramite'] == 7){
				\Log::info("MM entra a 1122");
				$tramite->tipo_terminal = $datos['tipo_terminal'];
				if($datos['id_tipo_tramite'] == 7)
					$tramite->retiro_definitivo = $datos['retiro_definitivo'];
			}
		...
		*/
		private function armaPlan($datos, $modifica){
			...
			$plan->superficie_valor = SuiteCRMRepo::obtenerSuperficiePorId($datos['superficie']);
				...
				// \log::info('MM- armaPlan datos.ubicacion:',array($datos['ubicacion']));
			$plan->ubicacion_codigo = $datos['ubicacion'];

			$plan->ubicacion_valor = SuiteCRMRepo::obtenerUbicacionPorId($datos['ubicacion']);
			// \log::info('MM- armaPlan plan.ubicacion_valor:',array($plan->ubicacion_valor));
		...
		*/
		private function armaTitular($tramite, $datos, $persona){
			$titular = new Titular();
			\Log::info("AE - servicios->Tramite - armaTitular - titular: ",array($titular));
			$datosAgencia = $this->obtenerDatosAgencia($tramite->nroPermiso);
			\Log::info("AE - servicios->Tramite - armaTitular - datosAgencia: ",array($datosAgencia));
			...
			if(isset($datosAgencia)){
				...
				// Modificado ADEN - 2024-09-03
				if(isset($datos['fecha_examen']) && $datos['fecha_examen']!='')
					$titular->fecha_examen = $this->formatos->fecha($datos['fecha_examen']);
				else
					$titular->fecha_examen = '0000-00-00';
				
				if(isset($datos['intentos']))
					$titular->intentos = $datos['intentos'];
				else
					$titular->intentos = 0;
				
				if(isset($datos['calificacion']))
					$titular->calificacion = $datos['calificacion'];
				else
					$titular->calificacion = 0;
					
				if($datos['tipo_tramite'] == 12 ){
					$titular->tipo_relacion = $datos['tipo_rel'];
					$titular->tipo_vinculo = $datos['tipo_vinh'];
				}else{
					$titular->tipo_relacion = 0;
					$titular->tipo_vinculo = 0;
				}
				return $titular;
			}else{
				return true;
		...
		//obtener el vinculo y relación
		public function obtenerRelyVin($nroTramite){
			$persona = TitularRepo::buscarPorNroTramite($nroTramite);
			\Log::info('MM - obtenerRelyVin - persona:',array($persona));
			// if(!is_null($persona)){
				// $persona_tipo_rel = $persona['tipo_relacion'];
				// $persona_tipo_vin = $persona['tipo_vinculo'];
			// }
			//	$persona = $this->servicioPersona->buscarPorId($id_persona);
			/*	
				if($persona['cuit']!=""){
		...
		* que le llegan por parámetro
		*/
		private function armaAdjunto($nroTramite,$permiso, $ruta, $idPref){
			...
			$adjuntoTramite->idTipoDoc = intval($idPref); // $tipo_docu
			return $adjuntoTramite;
		}
		
		/**
		* Función que arma la entidad adjunto con los datos
		* que le llegan por parámetro
		*/
		private function armaAdjuntoConTipoDoc($nroTramite,$permiso, $ruta, $idTipoDoc = null){
			$adjuntoTramite = new AdjuntoTramite();
			$adjuntoTramite->nroTramite = $nroTramite;
			$adjuntoTramite->rutaAdjunto = $ruta;
			$adjuntoTramite->permiso = $permiso;
			$adjuntoTramite->idTipoDoc = $idTipoDoc; // $tipo_docu
			return $adjuntoTramite;
		}
		...
		*************************************************************/
		public function incorporarMaquina($tipo_usuario, $tipo_terminal, $observaciones){
			\Log::info("MM - servicio incorporarMaquina - tipo_terminal.",array($tipo_terminal));
			$datos['id_tipo_tramite']=6;
			if($tipo_usuario == 'CAS' || $tipo_usuario == 'CAS_ROS')
				$datos['id_estado_tramite']=10;
			else
				$datos['id_estado_tramite']=0;
			$datos['tipo_terminal']=$tipo_terminal;
			$datos['observaciones']=$observaciones;
			\Log::info("MM - servicio incorporarMaquina - datos: ",array($datos)); 
			
			$tramite = self::armaTramite($datos);
			\Log::info("MM - servicio incorporarMaquina - tramite: ",array($tramite));
			$tramite = TramiteRepo::crear($tramite);
			\Log::info("MM - servicio incorporarMaquina - 2tramite: ",array($tramite));
			/**datos necesarios para el email**/
			$datos_mail['estado_tramite']=$datos['id_estado_tramite'];
			$datos_mail['nroTramite']=$tramite->nroTramite;
			$datos_mail['tipoTramite']=$tramite->tipoTramite['nombre_tramite'];
			$datos_mail['agente']=$tramite->agente;
			$datos_mail['subagente']=$tramite->subAgente;
			$datos_mail['titular']=$tramite->titular;
			$datos_mail['nro_permiso']=$tramite->nroPermiso;
			...
			\Log::info("MM - servicio incorporarMaquina - datos_mail: ",array($datos_mail));
			//coloco en el historial el trámite
			$datos['estadoIni']=$datos['id_estado_tramite'];
		...
		***************************************************/
		public function retirarMaquina($id_terminal, $observaciones, $retiro_definitivo){
			$datos['id_tipo_tramite']=7;
			$datos['id_estado_tramite']=10;
			$datos['tipo_terminal']=$id_terminal;
			$datos['observaciones']=$observaciones;
			$datos['retiro_definitivo']=$retiro_definitivo;
			$tramite = self::armaTramite($datos);
			$tramite = TramiteRepo::crear($tramite);
			...
			$datos_mail['agente']=$tramite->agente;
			$datos_mail['subagente']=$tramite->subAgente;
			$datos_mail['razonSocial']=$tramite->razonSocial;
			$datos_mail['titular']=$tramite->titular;
			$datos_mail['nro_permiso']=$tramite->nroPermiso;
...
/*******************************PERMISOS**************************/
	/*****************Baja de permisos*************/
		public function solicitudBaja($permiso,$motivo,$observaciones, $usuarioTramite){
			...
			// \Log::info('MM - tramite->nroTramite: ',array($tramite->nroTramite));
			// \Log::info('MM - datos: ',array($datos));
			$this->servicioHistorial->crear($tramite->nroTramite, $datos);
			return $tramite;
		}
		/*****************Renuncia*************/
		public function solicitudRenuncia($permiso,$motivo,$observaciones, $usuarioTramite){
			$datos['id_tipo_tramite']=13;
			$datos['id_estado_tramite']=0;
			$datos['id_motivo_baja']=$motivo;
			$datos['observaciones']=$observaciones;
			$datos['permiso']=$permiso;
		...
		**/
		...
		public function existeSolicitudRenuncia($permiso){
			$ok=TramiteRepo::existeSolicitudRenuncia($permiso);
			return $ok;
		}
	...
	*/
	public function modificarTramite($datos){
		$actualizado = false;
		\Log::info("AE - Tramites - modificarTramite - datos: ",array($datos));
		switch ($datos['tipo_tramite']) {
			...
			case '2':
				...
				$datos_categoria['nro_red_nueva'] = $datos['nro_red'];
				$datos_categoria['nro_pto_vta_nuevo'] = $datos['nro_subagente'];
			...
			case '4':
				...
				break;
			// Agregado - ADEN - 2024-09-30
			case '11': // Baja de cotitular
			    // Modifico la 
				TramiteRepo::modificarObservaciones($datos['nro'], $datos['motivo_ct']);
				// Actualizar el cotitular a eliminar
				$datos_titular['id_titular_nuevo'] = $datos['cotitular_to_kill'];
...
*/
			...
			default:
				...
				if($datos['tipo_tramite'] == 12){
					$datos_titular['tipo_relacion']=$datos['tipo_rel'];
					$datos_titular['tipo_vinculo']=$datos['tipo_vinh'];
				}
				TitularRepo::modificarCampos($datos['nro'], $datos_titular);
				...
				if(!is_null($idPersona)){
					...
					// ini - ADEN - 2024-10-18, agrego tipo de documento, numero de documento y sexo!
					$datos_persona['tipo_documento'] = $datos['tipo_doc'];
					$datos_persona['nro_documento'] = $datos['nro_doc'];
					$datos_persona['sexo'] = $datos['sexo_persona'];
					// fin - ADEN - 2024-10-18, agrego tipo de documento, numero de documento y sexo!				
					
					$datos_persona['situacion_ganancia'] = $datos['tipo_situacion'];
					// ADEN - 2024-08-06, si no llegó el dato, lo inicializo en blanco!
					if (!isset($datos['ingresos'])) {
						$datos['ingresos'] = "";
					}
	...
	**/
	public function solicitudHabilitacion($permiso,$fechadesde,$observaciones, $usuarioTramite){
		...
		/*INI: para evitar errores con datos de session erróneos del usuario, se refrescan los mismos*/
		\Session::forget('usuarioTramite');
		$usuario=$this->controladorUsuario->buscarPorPermiso($permiso);
		// \Log::info("MM - usuario", array($usuario));
		$respuesta = \Response::json(array('usuario'=>$usuario));
		\Session::put('usuarioTramite',$usuario);
		$usuarioTramite=\Session::get('usuarioTramite');
		/*FIN: para evitar errores con datos de session erróneos del usuario, se refrescan los mismos*/
...
//		$evaluacion->estado_local = $datos['estado_local'];
		if(!empty($datos['estado_local'])):
			$evaluacion->estado_local = $datos['estado_local'];
		else:
			$evaluacion->estado_local = 0; 
		endif;
// FIN MODIFICADO ADEN - 2024-04-25		
		
// INI MODIFICADO MM - 2024-03-25		
		// $evaluacion->estado_cuantitativo = $datos['estado_cuantitativo'];
		$evaluacion->estado_cuantitativo = 0;
// INI MODIFICADO MM - 2024-03-25		
// INI MODIFICADO ADEN - 2024-04-25		
//		$evaluacion->estado_entorno = $datos['estado_entorno'];
		if(!empty($datos['estado_entorno'])):
			$evaluacion->estado_entorno = $datos['estado_entorno'];
		else:
			$evaluacion->estado_entorno = 0; 
		endif;
// FIN MODIFICADO ADEN - 2024-04-25		

// INI MODIFICADO ADEN - 2024-04-25		
//		$evaluacion->estado_competencia = $datos['estado_competencia'];
		if(!empty($datos['estado_competencia'])):
			$evaluacion->estado_competencia = $datos['estado_competencia'];
		else:
			$evaluacion->estado_competencia = 0; 
		endif;
// FIN MODIFICADO ADEN - 2024-04-25		
		...
		$evaluacion->observacion_cuantitativo = "";
...
//\Log::info("nuevoTodoCompleto: ", array($nuevoTodoCompleto));
	...
	/* Baja de cotitular */
	public function obtenerCotitulares($nro_permiso){
		return CotitularRepo::obtenerCotitulares($nro_permiso);
	}

	public function obtenerCotitularTramite($nro_tramite){
		$persona = TitularRepo::buscarPorNroTramite($nro_tramite);
		if(!is_null($persona)){
			$id_persona = $persona['id_titular_nuevo'];
			$result = PersonaRepo::buscarPersonaSuite($id_persona);
			return $result;
		}
		return null;
	}

	public function actualizarTramite($motivo, $tipoTramite){
		$datos['id_estado_tramite'] = 0;
		$datos['id_tipo_tramite'] = $tipoTramite;
		$datos['observaciones'] = $motivo;
		if(\Session::has('tramiteNuevoPermiso')){ //FIXME: ya existe el trámite, nombre confuso
			$tramiteNuevoPermiso=\Session::get('tramiteNuevoPermiso');
			$tramite = TramiteRepo::buscarPorId($tramiteNuevoPermiso['nro_tramite']);
			TramiteRepo::modificarCampoDetalleCompleto($tramite->nroTramite, 1);
			TramiteRepo::modificarObservaciones($tramite->nroTramite, $datos['observaciones']);
		...
		\Log::info("AE - tramitesController - actualizarTramite - tramite: ",array($tramite));

		return $tramite;
	}

	public function actualizarTitular($tramite, $datos, $id_nuevo_titular){
		
		$titular = self::armaTitular($tramite, $datos, array('id' => $id_nuevo_titular));
		\Log::info("MM - tramitesController - actualizarTitular - titular: ",array($titular));
		if(isset($titular))
			$titular = TitularRepo::cargarTitular($titular);
		return $titular;
	}
