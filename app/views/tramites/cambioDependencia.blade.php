...
@section('javascript')
	{{HTML::script("js/js-localidades.js?v=".rand(), array("type" => "text/javascript"))}}
	{{HTML::script("js/js-cambiodependencia.js?v=".rand(), array("type" => "text/javascript"))}}	
...
@section('contenido')
	...
	<div class="row-border">
		@if(Session::has('tramiteNuevoPermiso'))
			...
			<BR>
		@endif
		{{Form::open(array('action' => array('TramitesController@cambioDependencia_add'), 'method' => 'post', 'id' => 'formulario_dependencia', 'class'=>'form-horizontal'))}} 
							...
							@if($meses==0)
								@if($dias==1)
									<div class="col-sm-6 div_sin_padding alert-danger text-center">
										{{Form::label('margen','IMPORTANTE: Último Cambio de Dependencia hace '.$dias.' dia.',array('id'=>'margen','class'=>'alert-danger'))}}
									</div>
								@elseif($dias==0)
									<div class="col-sm-6 div_sin_padding alert-danger text-center">
									{{Form::label('margen','IMPORTANTE: Último Cambio de Dependencia hoy.',array('id'=>'margen','class'=>'alert-danger'))}}
									</div>
								@else
									<div class="col-sm-6 div_sin_padding alert-danger text-center">
									{{Form::label('margen','IMPORTANTE: Último Cambio de Dependencia hace '.$dias.' dias.',array('id'=>'margen','class'=>'alert-danger'))}}
									</div>
								@endif
							@elseif($meses==1)
								<div class="col-sm-6 div_sin_padding alert-danger text-center">
									{{Form::label('margen','IMPORTANTE: Último Cambio de Dependencia hace '.$meses.' mes.',array('id'=>'margen','class'=>'alert-danger'))}}
								</div>
							@else
								<div class="col-sm-6 div_sin_padding alert-danger text-center">
									{{Form::label('margen','IMPORTANTE: Último Cambio de Dependencia hace '.$meses.' meses.',array('id'=>'margen','class'=>'alert-danger'))}}
								</div>
							@endif				
					@endif
				...
				<div class="alert alert-warning col-sm-12" id="nueva_red">
					<h4>{{Form::label('lred_destino', 'Red Destino', array('id'=>'lred_destino','class'=>'text-center col-xs-1 col-lg-12'))}}</h4>
					
					<div class="form-group">	
						{{Form::label('Nº Red:', '', array('id'=>'lnro_red','class'=>'control-label col-xs-3'))}}
						<div class="col-sm-2">
							{{Form::text('nro_red', '', array('id' => 'nro_red', 'class'=>'form-control ' ,'maxlength' => '4', 'data-toggle'=>'tooltip', 'data-placement'=>'right', 'autofocus'=>"", 'onfocus'=>'siguienteCampo="nro_subagente"; nombreForm=this.form.id'))}} 
						</div>
						<div class="col-sm-6" id="div_razon_social" style="display: none">
								{{Form::text('razon_social_nueva_red', '', array('id'=>'razon_social_nueva_red','class'=>'form-control col-xs-3', 'readonly'=>'readonly'))}} 
						</div>
					</div>
					<div class="form-group">
						{{Form::label('lnro_subagente', 'Subagente:', array('id'=>'lnro_subagente', 'class'=>'control-label col-xs-3 col-lg-3'))}}
						<div class="col-sm-2 col-lg-2">
							{{Form::text('nro_subagente', '', array('id' => 'nro_subagente', 'class'=>'form-control col-xs-3 text-right' ,'maxlength' => '10', 'data-toggle'=>'tooltip','onfocus'=>'siguienteCampo="buscarLocalidad"; nombreForm=this.form.id'))}}  
						</div>
					...
					<div id="div_nueva_localidad" class="form-group">
						...
						<div class="col-sm-5">
							<input list="localidades" name="buscarLocalidad" id="buscarLocalidad" class='form-control' placeholder="Ingrese Localidad" autocomplete="off" onfocus='siguienteCampo=this.form.domicilio.id; nombreForm=this.form.id'>
					...
					<div class="form-group">	
						{{Form::label('Domicilio Actual Agencia:', '', array('id'=>'ldomicilio','class'=>'control-label col-xs-8 col-lg-3'))}}
						<div class="col-sm-5">  
						{{Form::textarea('domicilio', '', array('id' => 'domicilio', 'class'=>"form-control input-medium",'maxlength'=>'255' , 'onfocus'=>'siguienteCampo = this.form.referente.id;nombreForm=this.form.id', 'col'=>'50', 'rows'=>'3'))}}
						</div>
					</div>	
					...
					<div class="form-group">	
						...
						<div class="col-sm-5">  
							{{Form::textarea('datos_contacto', '', array('id' => 'datos_contacto', 'class'=>"form-control input-medium",'maxlength'=>'255' ,'onfocus'=>'siguienteCampo = this.form.motivo_cr.name;nombreForm=this.form.id', 'col'=>'50', 'rows'=>'3'))}}
						</div>
					</div>	
					
					<div class="form-group">
					
						{{Form::label('lmotivo', 'Motivo:', array('id'=>'lmotivo_cambior','class'=>'control-label col-xs-1 col-lg-3'))}}
				
						<div class="col-sm-4 col-lg-8">
							{{Form::textarea('motivo_cr', '', array('id' => 'motivo_cr', 'class'=>"form-control input-medium", 'maxlength' => '255', 'onfocus'=>'siguienteCampo = "btn_adjuntar";nombreForm=this.form.id'))}}
						</div>
			...
			<div class="col-sm-6 col-lg-6 ">
				...
				{{Form::button('Adjuntar archivos', array('id' => 'btn_adjuntar', 'name'=>'btn_adjuntar', 'class' => 'col-sm-3 glyphicon glyphicon-plus btn btn-success', 'onfocus'=>'siguienteCampo = "btn_iniciar";nombreForm=this.form.id'))}}
				<div id="adjuntos" style="display:none" class="col-sm-9">
						{{Form::hidden('nombre_tipo_tramite','CR',  array('id' => 'nombre_tipo_tramite'))}}
		...
		<div class="col-sm-12">		
			...
			{{-- Form::button('Ingresar', array('id' => 'btn_iniciar', 'disabled'=>$bloqIngreso, 'name'=>'btn_iniciar', 'class' => 'btn-primary')) --}} 
			{{ Form::button('Ingresar', array('id' => 'btn_iniciar',  'name'=>'btn_iniciar', 'class' => 'btn-primary')) }} 
