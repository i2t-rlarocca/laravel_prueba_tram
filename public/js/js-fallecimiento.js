...
jQuery(document).ready(function(){
	...
	jQuery('#tipo_vin').hide();
	jQuery('#ltipo_vin').hide();
...
/*************************************************************************/
		jQuery('#tipo_rel').on('change', function(){
			...
			if(trval2 == 1){
				jQuery('#tipo_vin').show();
				jQuery('#ltipo_vin').show();
    ...
    jQuery("#formulario-fallecimiento").validate({
      ignore: "",
      focusInvalid: false,
      ...
      rules: {
            ...
            razon_social: {required:function(){
                            var tp= jQuery('#tipo_persona').val();
                            if(tp=="F"){
                              return false;
                            }else{
...
*/			
      ...
      messages: {
            cuit: {required:"Debe ingresar cuit.", cuit:"No válido"},
...
/**************************************************/
jQuery('#formulario-fallecimiento').on('submit', function() {
...
jQuery.fn.submitFormTitular = function(){
        ...
        if(id!=""){
          ...
          var valido = jQuery('#formulario-fallecimiento').validate().form();
          var nl = jQuery('input[name="nueva_localidad"]').valid();

          if(valido && nl){
            jQuery("#formulario-fallecimiento :disabled").removeAttr('disabled');
            ...
            jQuery('#formulario-fallecimiento').submit();
          }else{
...
jQuery('#formulario-fallecimiento').on('keydown click','#btn_ingresar',function(e){
        ...
        if(jQuery('#tipo_persona').val()=='F'){
          ...
          if(jQuery('#cuit').val().length>0){
            if(!jQuery('#cuit').is(':disabled')){
              ...
              jQuery(this).habiReset();
              //jQuery('#formulario-fallecimiento').trigger("reset");
            }//cuit disabled
          }else{
            jQuery(this).buscarPersona(doc,tipo);
          }
...
/**
  ...
  **/
  jQuery.fn.habiReset=function(){
          ...
          if(tipo_persona=='F'){
// INI MODIFICADO - ADEN - 2024-07-19
...
// FIN MODIFICADO - ADEN - 2024-04-19
  ...
  **/
  ...
  jQuery('#id_email').on('change', function(){
	...
	jQuery('#formulario-fallecimiento').on('keydown click','#btn_cargarPlan',function(e){
	//alert('llega1');
        var code = e.keyCode || e.which;
        if(e.type == "keydown"){
           if(code == 13) { //Enter keycode
 ...
 function refrescar(formulario)
{
   ...
   jQuery('#formulario-fallecimiento')[0].reset();
}
