...
//window.onload=
function convertirNumLetras(opc)

//parametro letras agregado por BOZAPE para decirle en que campo del form se asigna el valor en letras 2013-08-03
{
	if(opc==1){
	var number = document.getElementById('retencion').value;
	var letras = 'monto_retencion';
	}
	if(opc==2){
	var number = document.getElementById('importe_neto').value;
	var letras = 'imp_net_letras';
	}
	...
	var moneda = " PESO "; // Agregado por BOZAPE para el nombre de la moneda en singular 2013-08-03
	var monedap = " PESOS ";  // Agregado por BOZAPE para el nombre de la moneda en plural 2013-08-03
	var monedad = " CENTAVOS "; // Agregado por BOZAPE para el nombre de la moneda para los decimales 2013-08-03
	var monedad0 = " PESOS "; // Agregado por BOZAPE para el nombre de la moneda para cuandon no hay decimales 2013-08-03
   ...
   cad = cad.toUpperCase();
   // hasta aqui viene todo correcto en cuanto ala cifra entera
   ...
   if (centavos == 0) { //agregado por BOZAPE para cuando no hay decimales entonces se usa el valor de moneda para no decimales MONEDAD0
           centavos = "";
           monedad = "";
   }
   else
   ...
   if (centavos.length==1)
   {
	...
	var centavosn = Number(centavos);
	var con= 'CON ';
   if(centavosn >= 10)
	   {
	   centavosdv = ObtenerParteEntDiv(centavos, 10);	   
	   centavos = string_literal_conversion(centavos);
	   centavos = centavos.toUpperCase();
	   centavos = con.concat(centavos);
	   }   

   if (centavosn < 10)
	   {
	   centavosdv = ObtenerParteEntDiv(centavos, 1);
	   centavos = string_literal_conversion(centavos);
	   centavos = centavos.toUpperCase();
  ...
  texto= cad+moneda+centavos+monedad; //modificado por BOZAPE en vez de poner el texto de moneda decimal se asigna la variable monedad 2013-08-03
  document.getElementById(letras).value= texto;  //modificado por BOZAPE en vez de devoler por funcion el valor en letras con RETURN se asigna al campo recibido por el parametro LETRAS
}